...
//!
//! 文件功能：
//! 构建 macOS 进程所有权树，从目标 PID 向上找出生命周期控制者。
//!
//! 主要职责：
//! - 向上最多 8 层，并防止 PID 循环
//! - 跳过 launchd / kernel_task / Terminal 等系统宿主
//!
//! 作者：Chushi Jack
//! 创建时间：2026-08-20
...
/// 父进程是否为 launchd。brew services / LaunchAgent 的子进程常见此形态。
pub fn parent_is_launchd(pid: u32) -> bool {
    ancestor_chain(pid)
        .and_then(|chain| chain.into_iter().nth(1))
        .map(|parent| parent.name.eq_ignore_ascii_case("launchd"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    #[test]
    fn skips_launchd_and_shells() {
        assert!(is_system_host("launchd"));
        assert!(is_system_host("kernel_task"));
        assert!(is_system_host("Terminal"));
